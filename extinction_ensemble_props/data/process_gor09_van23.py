...
if __name__ == "__main__":
    tab1 = QTable.read(
        "Gordon09/apj308925t2_ascii.txt", format="ascii.commented_header"
    )
    tab2 = QTable.read(
        "Gordon09/apj308925t3_ascii.txt", format="ascii.commented_header"
    )
    ...
    for ccol, ctab in zip(colnames, tabnames):
        ...
        otab[f"{ccol}_unc"] = np.sqrt(
            tab1[f"{ctab}_unc1"].data ** 2 + tab1[f"{ctab}_unc2"].data ** 2
        )
    ...
    for ccol, ctab in zip(colnames, tabnames):
        ...
        otab[f"{ccol}_unc"] = np.sqrt(
            tab2[f"{ctab}_unc1"].data ** 2 + tab2[f"{ctab}_unc2"].data ** 2
        )
